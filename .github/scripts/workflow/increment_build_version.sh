...
# Set up Git with the smarterweather account
git config --local user.email "smarterweather@gmail.com"
git config --local user.name "smarterweather"
...
git push https://${GITHUB_ACTOR}:${SMARTERWEATHER_PAT}@${GITHUB_SERVER_URL#https://}/$GITHUB_REPOSITORY.git "$NEW_BRANCH"
...
PR_RESPONSE=$(curl -s -X POST -H "Authorization: token $SMARTERWEATHER_PAT" \
...
curl -s -X PATCH -H "Authorization: token $SMARTERWEATHER_PAT" \
    -H "Content-Type: application/json" \
